...
| BackupManager    | ⚠️     | Файлы и БД работают; Restic upload — не работает (требует настройки окружения) |
...
- **Restic backup** не загружает данные в удалённое хранилище — требуется настройка переменных окружения `RESTIC_REPOSITORY` и `RESTIC_PASSWORD`
