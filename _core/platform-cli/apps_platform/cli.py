...
def get_all_running_containers() -> set:
    """Получить все запущенные контейнеры одним запросом"""
    try:
        result = subprocess.run(
...
def get_service_status_fast(service_name: str, running_containers: set) -> str:
    """Быстрая проверка статуса сервиса по имени контейнера"""
    # Проверяем по точному совпадению или по префиксу
    if service_name in running_containers:
        return "running (1)"
    # Ищем контейнеры с именем, начинающимся с имени сервиса
    matches = [c for c in running_containers if c.startswith(service_name)]
    if matches:
        return f"running ({len(matches)})"
    return "stopped"
...
def list(
    ...
    status: str | None = typer.Option(None, "--status", "-s", help="Фильтр по статусу (running/stopped)"),
):
    """Показать все сервисы"""
    services = get_services()

    # Получаем все запущенные контейнеры одним запросом (O(1) вместо O(n))
    ...
    for name, info in sorted(services.items()):
        ...
        # Быстрая проверка статуса (O(1) для каждого сервиса)
        svc_status = get_service_status_fast(name, running_containers)
        if status:
...
def new(
    ...
    # Валидация имени сервиса от path traversal
    if name.startswith('/') or '..' in name or any(c in name for c in '/\\'):
...
def main():
    """Точка входа CLI"""
    global PROJECT_ROOT
    PROJECT_ROOT = Path(os.getenv("OPS_PROJECT_ROOT", Path.cwd()))
    app()
