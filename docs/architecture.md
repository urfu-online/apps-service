...
**Kopia** — современная система резервного копирования с дедупликацией, шифрованием и поддержкой множества бэкендов (файловая система, S3, SFTP, Google Cloud Storage, Azure Blob).
...
- **Гибкие политики хранения** — автоматическое удаление старых снапшотов по правилам (keep-daily, keep-weekly, keep-monthly, keep-annual)
- **Уведомления** — интеграция с ntfy.sh/Apprise для отправки уведомлений о статусе бэкапов
- **Веб-интерфейс** — встроенный UI Kopia для просмотра и управления бэкапами

Архитектура:

- **Kopia Server** — отдельный контейнер (`_core/kopia/`), предоставляет API для операций с бэкапами
- **Kopia CLI** — используется скриптами бэкапа для создания и восстановления снапшотов
- **Хранилище** — по умолчанию локальная файловая система (`/repository`), поддерживается S3, SFTP и другие бэкенды
- **Политики** — настраиваются через `kopia_policy` в конфигурации сервиса

Интеграция с платформой:

- Расположение: `_core/kopia/`
- Скрипты: `_core/backup/scripts/kopia_*.sh` (backup, restore, policy)
- Переменные окружения: `KOPIA_REPOSITORY`, `KOPIA_REPOSITORY_PASSWORD`, `KOPIA_STORAGE_TYPE`
...
graph TB
    ...
    subgraph "Ядро платформы"
        ...
        Kopia[Kopia<br/>Резервное копирование]
    end
    ...
    %% Связь сервисов с хранилищем (бэкапы)
    ServiceA -.-> Kopia
    ServiceB -.-> Kopia
    ServiceC -.-> Kopia
    ServiceD -.-> Kopia
    ...
    class Master,Caddy,Keycloak,Kopia core
...
5. Запускает бэкапы через Kopia (по расписанию или вручную)
6. Отправляет уведомления через ntfy.sh/Apprise

### Поток данных:

- Внешний трафик `Caddy` нужный сервис (внутри Docker network)
- Логи сервисов ` собираются Master Service
- Данные бэкапов ` хранятся в Kopia репозитории (локально или удалённо: S3, SFTP и т.д.)

## 5. Актуальная файловая структура

```shell
apps/
├── backups/                     # Устаревшие локальные бэкапы (больше не создаются)
├── _core/
│   ├── backup/                  # Скрипты бэкапов для Kopia (kopia_*.sh)
│   │   ├── docker-compose.yml
│   │   ├── scripts/             # Скрипты создания/восстановления для Kopia
│   │   └── schedules/           # Расписания (cron-like) для запуска бэкапов
│   ├── caddy/                   # Caddy Proxy
│   │   ├── Caddyfile
...
| Backup        | Kopia (дедупликация, шифрование, поддержка S3/SFTP) |
