...
<!-- КОММЕНТАРИЙ: Поле `auto_subdomain` генерирует домен вида `{name}.{base_domain}`, но неясно, требуется ли также указывать `type: domain` или это отдельный тип маршрутизации. В коде проверяется `route.auto_subdomain` и строится домен. Лучше добавить пример. -->
